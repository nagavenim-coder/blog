...
class ShowTheme
    ...
    field :is_red_hot, type: Boolean
    field :is_google_watch_feed, type: Boolean
...
class MovieBlogGenerator
  ...
  def search_movie_details(title,show)
    ...
    begin
      ...
      return default_movie_data(title,show) unless response.success?
  ...
  def default_movie_data(title,show)
    ...
    ai_data || {
      ...
      content_rating: 'U/A',
      poster_url: nil,
      watch_url: "https://shemaroome.com/shows/#{show}/#{title.downcase.gsub(' ', '-')}"
    }
  end
  ...
  def generate_blogs
    ...
    EpisodeTheme.where(:status => "published",:business_group_id => "548343938",:app_ids => "350502978",:show_name => s.title).to_a.each do |movie|
